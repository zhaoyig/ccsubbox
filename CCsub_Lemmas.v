...
Proof with simpl_env; eauto*.
  ...
  - Case "typing_tapp".
    ...
    { enough (PurePT : pure_type (∀ [P] T)) by (inversion PurePT; assumption).
      enough (TypeCPT : type (C # ∀ [P] T)) by (inversion TypeCPT; subst; [ inversion H0 | assumption ]).
      eapply type_from_wf_typ, Hwf.
    }
    assert (PureQ : pure_type Q) by (apply (proj1 (sub_pure_type _ _ _ _ Sub) PureP)).
    repeat split...
    apply wf_typ_open_type with (R := Q); inversion Hwf; subst...
    apply ok_from_wf_ctx in R1...
    inversion Hwf; subst...
    inversion H8; subst...
    apply wf_typ_all with (L := L)...
    intros. specialize (H12 X ltac:(fsetdec)).
    apply (wf_typ_ignores_sub_bindings P Q _ _ nil X S H12).
  - Case "typing_box".
    repeat split...
    apply wf_typ_capt...
    apply type_box.
