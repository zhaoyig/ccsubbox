...
Hint Resolve env_well_typed_uniq env_well_typed_store_ctx_wf env_well_typed_ctx_wf : core.

Lemma env_typing_deterministic : forall S E Γ1 Γ2,
  env_well_typed S E Γ1 ->
  env_well_typed S E Γ2 ->
  Γ1 = Γ2.
Proof with eauto.
  intros * EnvTyp1 EnvTyp2.
  generalize dependent Γ2.
  dependent induction EnvTyp1; intros Γ2 EnvTyp2; inversion EnvTyp2; subst...
  unshelve epose proof (StoreImpl.binds_unique _ _ _ _ _ H0 H8 _)...
  inversion H1; rewrite (IHEnvTyp1 _ H5)...
Qed.
...
Proof with eauto.
  ...
  - eapply fv_loc_transform_nil with (T1 := U)...
  - applys sub_regular H4.
Qed.
...
Hint Resolve uniq_from_wf_store_ctx typed_store_ctx_wf : core.

Lemma typed_store_uniq : forall SS S,
  store_typing SS S ->
  StoreImpl.uniq SS.
Proof with eauto.
  intros. dependent induction H...
...
Lemma env_well_typed_weakening_store : forall Γ S1 S2 S3 E,
...
Lemma frame_typing_inversion : forall S e E U,
  frame_typing S (e, E) U ->
  exists Γ T T',
    typing Γ S e T /\ env_well_typed S E Γ /\ loc_transform E T T' /\ sub nil S T' U.
Proof with eauto using sub_reflexivity, frame_typing_regular.
  intros * FrameTyp.
  dependent induction FrameTyp; intros; subst.
  - exists Γ, U, T; repeat split...
    eapply sub_reflexivity...
  - epose proof (IHFrameTyp e E ltac:(auto)) as [Γ [T0 [T' [Typ [EnvTyp [LocTrans Sub]]]]]].
    exists Γ, T0, T'; repeat split...
    apply sub_transitivity with (Q := U)...
Qed.

Lemma frame_typing_weakening_store : forall S1 S2 S3 e E T,
  frame_typing (S1 ++ S3) (e, E) T ->
  wf_store_ctx (S1 ++ S2 ++ S3) ->
  frame_typing (S1 ++ S2 ++ S3) (e, E) T.
Proof with eauto using env_well_typed_weakening_store, typing_weakening_store, sub_weakening_store, sub_reflexivity.
  intros * FrameTyp WfStore.
  dependent induction FrameTyp; intros; subst.
  - econstructor...
...
Lemma store_typing_inversion : forall S SS E l v C R,
  store_typing SS S ->
  StoreImpl.binds l (C # R) S ->
  stores l (v, E) SS ->
  frame_typing S (v, E) (C # R) /\ value (v, E).
Proof with eauto using typing_weakening_store, frame_typing_weakening_store, sub_reflexivity.
  intros * StoreTyp lBindsU lBindsv.
  generalize dependent l. generalize dependent C.
  generalize dependent v. generalize dependent E.
  generalize dependent R.
  dependent induction StoreTyp; intros.
  { inversion lBindsU. }
  ...
  rewrite_env ([(l, (C # R))] ++ S) in lBindsU.
  StoreImpl.analyze_binds lBindsU; subst.
  all: rewrite_env ([(l, store (v, E))] ++ SS) in lBindsv;
       unfold stores in lBindsv; StoreImpl.analyze_binds lBindsv; subst.
  all: assert (wf_store_ctx ([(l, C # R)] ++ S)) by (constructor; eauto).
  - inversion BindsTacVal; subst.
    inversion select (store _ = store _); subst; split; auto.
    rewrite_env (nil ++ [(l, C # R)] ++ S)...
  - apply StoreImpl.binds_In in BindsTac.
    erewrite (store_typing_preserves_dom SS S) in BindsTac...
    flsetdec.
  - apply StoreImpl.binds_In in BindsTac.
    flsetdec.
  - destruct (IHStoreTyp _ _ _ _ _ BindsTac BindsTac0) as [FrameTyp Value].
    split; auto.
    rewrite_env (nil ++ [(l, (C # R))] ++ S)...
Qed.
...
Lemma runtime_ctx_binds_loc : forall Γ E S x C R,
  ...
  exists l D, binds x l E /\ C = cse_loc l /\ StoreImpl.binds l (D # R) S.
Proof with eauto.
  intros * EnvTyp Binds.
  ...
  analyze_binds Binds; subst; simpl in *.
  - inversion select (_ = _); subst.
    exists l, C0; repeat split...
  - destruct (IHEnvTyp BindsTac) as [l0 [D [Binds0 [Eq StoreBinds]]]]; subst.
    exists l0, D; repeat split...
Qed.
...
Lemma env_well_typed_app : forall S E1 E2 Γ,
  env_well_typed S (E1 ++ E2) Γ ->
  exists Γ1 Γ2, Γ = Γ1 ++ Γ2 /\  env_well_typed S E2 Γ2.
Proof with eauto.
  intros * EnvTyp.
  dependent induction EnvTyp; simpl...
  - exists (@nil (atom*binding)), (@nil (atom*binding)); split...
    destruct E1...
  ...
  - destruct E1; simpl in *...
    + destruct E2; simpl in *; inversion x; subst...
      unshelve epose proof (IHEnvTyp nil E2 _) as [Γ1 [Γ2 [Eq EnvTyp2]]]; subst...
      exists (@nil (atom*binding)), ((x0, bind_typ (cse_loc l # R)) :: Γ2); split...
      2: econstructor...
      enough (Γ1 = nil); subst; simpl...
      epose proof (env_typing_deterministic _ _ _ _ EnvTyp2 EnvTyp) as Eq'; subst...
      symmetry in Eq'.
      epose proof (list_app_ident_r _ _ _ Eq') as Eq''; subst...
    + inversion x; subst...
      unshelve epose proof (IHEnvTyp E1 E2 _) as [Γ1 [Γ2 [Eq EnvTyp2]]]; subst...
      exists ((x0, bind_typ (cse_loc l # R)) :: Γ1), Γ2; split...
Qed.

Lemma env_well_typed_strengthening : forall S E1 E2 Γ1 Γ2 x l R,
  env_well_typed S E2 Γ2 ->
  env_well_typed S (E1 ++ [(x, l)] ++ E2) (Γ1 ++ [(x, bind_typ (cse_loc l # R))] ++ Γ2) ->
  env_well_typed S (E1 ++ E2) (Γ1 ++ Γ2).
Proof with eauto using dom_app.
  intros * EnvTyp2 EnvTyp.
  dependent induction EnvTyp; subst...
  - epose proof (list_app_nil _ _ _ x) as [Eq1 Eq2]; subst...
    symmetry in Eq2.
    epose proof (list_app_nil _ _ _ Eq2) as [Eq1' Eq2']; subst...
    inversion Eq1'.
  - induction E1; simpl in *; inversion x2; subst...
    + destruct Γ1; simpl in *...
      inversion x; subst.
      rewrite dom_app in H.
      simpl in H...
      clear - H; exfalso; fsetdec.
    + destruct Γ1; simpl in *; inversion x; subst...
      * assert (x1 `in`A dom (E1 ++ (x1, l) :: E2)) as InX.
        { rewrite dom_app; simpl... }
        rewrite (env_well_typed_preserves_dom _ _ _ EnvTyp) in InX.
        fsetdec.
      * econstructor...
Qed.

Lemma runtime_ctx_no_fv_cctx: forall S E Γ,
  env_well_typed S E Γ ->
  forall x, x `notin`A fv_cctx Γ.
Proof with eauto using wf_typ_from_wf_store_ctx_nil, wf_typ_notin_fv_ct.
  intros * EnvTyp x NotIn.
  induction EnvTyp; simpl in *...
  - fsetdec.
  - assert (x `in`A fv_ct R \/ x `in`A fv_cctx Γ) as [InR | InCctx] by fsetdec...
    assert (wf_typ nil S (C # R))...
    inversion H1; subst...
    eapply wf_typ_notin_fv_ct in H7...
Qed.

Lemma env_well_typed_weakening : forall S E1 E2 Γ1 Γ2 z l C R,
  StoreImpl.binds l (C # R) S ->
  env_well_typed S (E1 ++ E2) (Γ1 ++ Γ2) ->
  (* This second condition guarantees we add at the same spot *)
  env_well_typed S E2 Γ2 ->
  z `notin`A dom E1 `union`A dom E2 ->
  env_well_typed S (E1 ++ [(z, l)] ++ E2) (Γ1 ++ [(z, bind_typ (cse_loc l # R))] ++ Γ2).
Proof with eauto.
  intros * Binds EnvTyp EnvTyp2 NotIn.
  dependent induction EnvTyp; subst...
  - epose proof (list_app_nil _ _ _ x) as [Eq1 Eq2]; subst...
    epose proof (list_app_nil _ _ _ x0) as [Eq1' Eq2']; subst...
    simpl_env...
    econstructor...
  - induction E1; simpl in *; inversion x1; subst...
    + destruct Γ1; simpl in *...
      * destruct Γ2; simpl in *; inversion x; subst...
        econstructor...
        rewrite (env_well_typed_preserves_dom _ _ _ EnvTyp) in NotIn...
      * inversion x; subst...
    ...
    + destruct Γ1; simpl in *; inversion x; subst...
      * exfalso.
        inversion EnvTyp2; subst.
        epose proof (env_well_typed_app _ _ _ _ EnvTyp) as [Γ1' [Γ2' [Eq EnvTyp2']]]; subst.
        inversion EnvTyp2'; subst.
        rewrite dom_app in H.
        simpl in H...
      * econstructor...
