...
Proof with eauto.
  ...
  - apply uniq_from_wf_ctx in WfCtx.
    ...
    + destruct (bind_typ_capt _ _ _ _ WfΓ BindsTac) as [C [R Eq]]; subst.
      unshelve epose proof (runtime_ctx_binds_loc Γ _ _ _ _ R) as [l [D [BindsL [EqL StoreBinds]]]]; subst...
...
Proof with eauto using loc_transform_cse_wf, loc_transform_ctx_wf, subcapt_reflexivity.
  ...
  - analyze_binds_uniq H; subst.
    ...
    + unshelve epose proof (runtime_ctx_binds_loc Γ _ _ _ R T) as [l [D [BindsL [EqL StoreBinds]]]]; subst...
      unshelve epose proof (loc_transform_cse_bound_fvar _ _ _ UniqE BindsL) as LocTransFvar.
      epose proof (loc_transform_cse_deterministic _ _ _ _ LocTransC1 LocTransFvar); subst.
      eapply IHSubcapt...
...
Proof with eauto using loc_transform_ctx_binds_typ, loc_transform_ctx_binds_sub, loc_transform_cse_wf, loc_transform_wf, loc_transform_pure, wf_typ_notin_fv_ct, wf_typ_from_wf_store_ctx_nil, loc_transform_ctx_wf, loc_transform_exp_fvar_like.
  ...
  - Case "typing_var".
    ...
    + erewrite <- env_well_typed_preserves_dom in H1...
      ...
      apply loc_transform_equiv...
      econstructor...
    + epose proof (runtime_ctx_binds_loc _ _ _ _ _ _ EnvTyp BindsTac) as [l [D [BindsL [EqL StoreBinds]]]]; subst...
...
Lemma loc_transform_subst_bound_var : forall E x y T U l,
  uniq E ->
  y `notin`A dom E ->
  binds x l E ->
  loc_transform E (subst_ct y (cse_loc l) T) U ->
  loc_transform E (subst_ct y (cse_fvar x) T) U.
Proof with eauto.
  intros * Uniq NotIn Binds LocTrans.
  assert (x <> y) as NotEq by (intro; apply binds_In in Binds; fsetdec).
  generalize dependent x.
  dependent induction LocTrans; subst; intros.
  { inversion Binds. }
  analyze_binds_uniq Binds; subst.
  - constructor...
    rewrite subst_ct_var...
  - constructor...
    simpl in NotIn.
    inversion Uniq; subst.
    assert (y `notin`A dom E) as NotInE...
    assert (x <> y) as NotEq2...
    assert (x <> x0) as NotEq3...
    replace (subst_ct x (cse_loc l0) (subst_ct y (cse_fvar x0) T))
      with (subst_ct y (cse_fvar x0) (subst_ct x (cse_loc l0) T)).
    2: {
      clear - NotEq NotEq2 NotEq3.
      induction T; simpl...
      destruct v...
      all: try f_equal...
      induction c; simpl...
      destruct (y == a); destruct (x == a); simpl...
      destruct (x == x0); fsetdec.
      destruct (y == a); destruct (x == x0); fsetdec.
      destruct (x == a); fsetdec.
      destruct (y == a); destruct (x == a); fsetdec.
      f_equal...
    }
    eapply (IHLocTrans _ _ _ H2 NotInE _ _ BindsTac NotEq).
    Unshelve.
    clear - NotEq NotEq2 NotEq3.
    induction T; simpl...
    destruct v...
    ...
    destruct (y == a); fsetdec.
    destruct (x == a); fsetdec.
    destruct (y == a); destruct (x == a); fsetdec.
    f_equal...
Qed.

Lemma loc_transform_mid : forall E1 x l E2 U T,
  uniq (E1 ++ [(x, l)] ++ E2) ->
  loc_transform (E1 ++ (x, l) :: E2) U T ->
  loc_transform (E1 ++ E2) (subst_ct x (cse_loc l) U) T.
Proof with eauto.
  intros * Uniq LocTrans.
  dependent induction LocTrans; subst; simpl in *.
  - exfalso.
    assert (x0 `in`A dom (E1 ++ (x0, l) :: E2)) as InX.
  ...
  - destruct E1; simpl in *...
    ...
    + destruct p; inversion x; subst...
      constructor...
      inversion Uniq; subst.
      rewrite subst_ct_double_loc...
Qed.

Lemma loc_transform_weakening_fresh : forall E1 E2 z l T U,
  z `notin`A fv_ct T ->
  loc_transform (E1 ++ E2) T U ->
  loc_transform (E1 ++ [(z, l)] ++ E2) T U.
Proof with eauto.
  intros * NotIn LocTrans.
  dependent induction LocTrans; subst; simpl in *.
  - epose proof (list_app_nil _ _ _ x) as [Eq1 Eq2]; subst...
    ...
    constructor...
    rewrite <- subst_ct_fresh...
  - destruct E1; simpl in *; inversion x; subst...
    + constructor...
      rewrite <- subst_ct_fresh...
    + constructor...
      eapply IHLocTrans...
      apply notin_fv_ct_subst_ct...
Qed.
