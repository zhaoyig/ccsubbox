(** Operations, lemmas, and tactics for working with environments,
    association lists whose keys are locs.  Unless stated otherwise,
...
(** An environment is a list of pairs, where the first component of
    each pair is an [loc].  We view the second component of each pair
    as being bound to the first component.  In a well-formed
    environment, there is at most one binding for any given loc.
...
(** The domain of an environment is the set of locs that it maps. *)

Fixpoint dom (E : list (loc * A)) : locs :=
  match E with
  | nil => empty
...
Fixpoint map (f : A -> B) (E : list (loc * A)) : list (loc * B) :=
  match E with
  | nil => nil
  | (x, V) :: E' => (x, f V) :: map f E'
  end.

(** [get] returns the value bound to the given loc in an environment
    or [None] if the given loc is not bound.  If the loc has
    multiple bindings, the one nearest to the head of the environment
    is returned. *)

Fixpoint get (x : loc) (E : list (loc * A)) : option A :=
  match E with
  | nil => None
  | (y,a) :: E' => if eq_loc_dec x y then Some a else get x E'
  end.
...
(** An environment is well-formed if and only if each loc is bound at
    most once. *)

Inductive ok : list (loc * A) -> Prop :=
  | ok_nil :
      ok nil
  | ok_cons : forall (E : list (loc * A)) (x : loc) (a : A),
...
Definition binds x b (E : list (loc * A)) :=
  get x E = Some b.
...
Definition singleton_list (A : Type) (x : loc * A) := x :: nil.

Lemma cons_concat : forall (A : Type) (E : list (loc * A)) x a,
...
Lemma dom_singleton_list : forall (A : Type) (x : loc) (a : A),
...
Hint Extern 1 (~ In _ _) => simpl_env in *; [lnotin_solve] : core.
...
Lemma ok_push :  forall (E : list (loc * A)) (x : loc) (a : A),
      ok E -> ~ In x (dom E) -> ok ([(x, a)] ++ E).
Proof.
  exact (@ok_cons A).
Qed.
...
(** A binding in the middle of an environment has an loc fresh from
...
Lemma binds_singleton : forall x a,
  binds x a [(x,a)].
Proof.
  intros x a. unfold binds. simpl. destruct (eq_loc_dec x x); intuition.
Qed.
...
Proof with auto.
  ...
  destruct (eq_loc_dec x y)... intros _ J. destruct J. flsetdec.
Qed.
...
Proof.
  ...
  discriminate.
  destruct (eq_loc_dec x y); intuition.
Qed.
...
Proof.
  unfold binds. simpl. intros. destruct (eq_loc_dec x y).
    split; congruence.
    discriminate.
Qed.
...
Proof with auto.
  ...
  destruct (eq_loc_dec x x); intuition.
  inversion Ok; subst. destruct (eq_loc_dec x z)...
    destruct H3. simpl_env. flsetdec.
Qed.
...
Proof with auto.
  ...
  destruct (eq_loc_dec z z). congruence. intuition.
  inversion Ok; subst. destruct (eq_loc_dec z x)...
    destruct H4. simpl_env. flsetdec.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H; subst. auto using union_3. flsetdec.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H. intuition. flsetdec.
Qed.
...
Proof.
  ...
  auto.
  inversion Ok; subst. destruct (eq_loc_dec x y); subst; auto.
...
Lemma binds_unique : forall E x B1 B2,
  ok E ->
  binds x B1 E ->
  binds x B2 E ->
  B1 = B2.
Proof with simpl_env in *; eauto.
  ...
  induction H...
  * binds_cases H0...
  * destruct (eq_loc_dec x x0); subst...
    binds_cases H0; binds_cases H1; subst; simpl_env in *; try solve [exfalso; lnotin_solve]...
    apply IHok...
    binds_cases H0...
    binds_cases H1...
Qed.
...
Lemma ok_ignores_binding : forall A (F E : list (loc * A)) x b1 b2,
  ok (F ++ [(x, b1)] ++ E) ->
  ok (F ++ [(x, b2)] ++ E).
Proof with eauto.
  intros*.
...
Lemma ok_tail : forall A (F E : list (loc * A)),
  ok (F ++ E) ->
  ok E.
Proof.
  intros.
...
Lemma tail_not_in_head : forall A (E F : list (loc * A)) x,
  ok (F ++ E) ->
  In x (dom E) ->
  ~ In x (dom F).
Proof.
