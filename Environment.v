...
Hint Extern 1 (~ In _ _) => simpl_env in *; [notin_solve] : core.
...
Proof with simpl_env in *; eauto.
  ...
  induction H...
  * binds_cases H0...
  * destruct (eq_atom_dec x x0); subst...
    binds_cases H0; binds_cases H1; subst; simpl_env in *; try solve [exfalso; notin_solve]...
    apply IHok...
    binds_cases H0...
    binds_cases H1...
Qed.
...
Lemma ok_ignores_binding : forall A (F E : list (atom * A)) x b1 b2,
  ok (F ++ [(x, b1)] ++ E) ->
  ok (F ++ [(x, b2)] ++ E).
Proof with eauto.
  intros*.
