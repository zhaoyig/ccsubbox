...
Proof with simpl_env;
  ...
  - assert (wf_ctx (Δ ++ [(Z, bind_sub Q)] ++ Γ) S) as WfC by auto.
    ...
    + apply (sub_transitivity Q)...
      * rewrite_env (nil ++ (map (subst_tb Z P) Δ) ++ Γ).
        apply sub_weakening...
      * rewrite (subst_tt_fresh Z P Q).
        ...
        destruct H...
        { exfalso. epose proof binds_dom_contradiction... }
        apply binds_app_1 in H...
        destruct H...
        apply binds_one_2 in H...
        inversion H; subst.
        apply (IHSsubT Q)...
        exfalso. epose proof binds_dom_contradiction...
    + analyze_binds H...
      ...
      * 
        (* assert (binds X (bind_sub U) (map (subst_tb Z P) Δ ++ Γ)) by admit. *)
        apply (sub_trans_tvar U)...
        rewrite (subst_tt_fresh Z P U).
        2: {
          assert (wf_typ Γ S U) as HA by (eapply wf_typ_from_binds_sub; eauto).
...
Proof with eauto.
  ...
  { apply uniq_from_wf_ctx in WfCtx.
    apply uniq_app_3 in WfCtx.
    unfold disjoint in WfCtx.
    simpl in WfCtx.
  ...
  - destruct (x0 == x); subst; try (exfalso; fsetdec).
    + rename select (binds x _ _) into Binds'.
      ...
      * inversion select (bind_typ _ = bind_typ _); subst.
        rewrite_env (nil ++ (map (subst_cb x (cse_fvar u)) Δ) ++ Γ).
        ...
        -- rewrite <- subst_ct_fresh. apply sub_reflexivity...
           auto.
      * exfalso. apply binds_In in BindsTac0; fsetdec.
    + rename select (binds x0 _ _) into Binds'.
      analyze_binds Binds'; destruct (x == x0); subst; simpl; destruct (x0 == x0); try fsetdec.
...
Proof with simpl_env;
  ...
  - assert (Z <> x).
    { destruct (typing_var_implies_binds_typ _ _ _ _ _ Typ2) as [C' [R' [Binds _]]].
      ...
      analyze_binds Binds; rename select (binds _ _ _) into Binds; apply binds_In in Binds; fsetdec.
    }
    simpl.
